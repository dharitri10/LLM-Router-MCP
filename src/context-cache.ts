...
export class ContextCache {
  ...
  update(sessionId: string, userPrompt: string, assistantResponse: string): void {
    ...
    log(`context updated — session="${sessionId}" turns=${turnCount} chars=${newEntry.length}`);
  }

  clear(sessionId?: string): void {
    if (sessionId) {
      this.sessions.delete(sessionId);
      log(`context cleared — session="${sessionId}"`);
    } else {
      this.sessions.clear();
      log("all contexts cleared");
    }
  }
