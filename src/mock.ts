...
const MOCK_RESPONSES: Record<TaskType, (prompt: string) => string> = {
  planning: (prompt) => `
...
export function mockClassify(prompt: string): TaskType {
  for (const [pattern, label] of KEYWORD_MAP) {
    if (pattern.test(prompt)) {
      log(`[MOCK] classified as: ${label}`);
      return label;
    }
  }
  log("[MOCK] classified as: general (fallback)");
  return "general";
}

// ── Mock model callers ───────────────────────────────────────────────────────

export async function mockCallClaude(prompt: string): Promise<string> {
  await delay(300);
...
export async function mockCallGemini(prompt: string): Promise<string> {
  await delay(200);
  const task = mockClassify(prompt);
  const handler = MOCK_RESPONSES[task] ?? MOCK_RESPONSES.general;
  return handler(prompt);
...
export async function mockCallGPT4o(prompt: string): Promise<string> {
  await delay(250);
  const task = mockClassify(prompt);
  const handler = MOCK_RESPONSES[task] ?? MOCK_RESPONSES.general;
  return handler(prompt);
